...
| Rule | Value |
|------|-------|
| Profit on an order | Sale price, less cost of stock, less that order's own expenses, less its share of the day's expenses |
| "The day's expenses" | The **daily** figure only — weekly, monthly and yearly overheads converted to their daily equivalent, then shared out across the day's orders in proportion to sales |
...
| **F3** | **The commission base includes allocated overheads — which is now confirmed correct.** `netSalesProfit = paidSalesReceived − stockCost − orderExpenses − globalExpenseAllocation − refunds − discounts`. The profit definition therefore stands unchanged. The consequence for K2 is timing, not arithmetic: overheads are allocated per calendar day in proportion to that day's sales, so an order's share is unknowable until the day's takings are known | `shared/reports/cashierShiftReport.ts:128`, `server/services/cashierShiftEngine.ts:247` |
...
**Answered — both former blockers are closed:**

1. **Overheads are in the commission base — the daily figure only.** (F3) Profit is the sale less
   stock cost, less that order's own expenses, less its share of the day's expenses.
   `dailyOverheadTotal` already converts weekly, monthly and yearly overheads to a daily
   equivalent, and `allocateGlobalExpenseShare` already shares that day's total by sales
   proportion — so **the profit definition in the code does not change at all.** K2 changes only
   who is paid and how it is split.

   The consequence K2 must absorb: an order's share of the day's expenses is not knowable at the
   moment of sale, because it depends on the day's total sales. Commission therefore accrues at
   **shift close**, not at order completion — which is where the allocation is already computed
   today. See K2 step 5.

2. **Partial payments release commission pro-rata**, and a settlement may be **split across tender
   types** — part cash, part card. (F5) Pay 40%, resolve 40% of the credit, accrue 40% of the
...
0. **Split tender at the point of sale.** "So much cash, so much card, so much on tick" may mean a
   ...
   settled in instalments. The credit schema already supports it (`amount_given` is a separate
   figure from the order total, so it can hold just the portion put on credit), but the till,
   `orders.payment_method` and the Z-report's payment-method breakdown all assume exactly one
   tender per order. Making that real needs an `order_payments` table and a split-tender POS
   screen — a brief of its own, and larger than anything in this phase. **K3 as written covers
   instalments against a debt; it does not cover splitting a sale at the counter.** Confirm which
   was meant before K3 starts.
...
- `+ migrations/052_commission_ledger.sql` — `cashier_commission_entries (id, org_id, order_id, cashier_id, role varchar(16) check (role in ('completer','inputter')), basis varchar(24) check (basis in ('sale','credit_resolution')), order_margin numeric(12,2), overhead_share numeric(12,2), commission_rate numeric(5,2), share_percent numeric(5,2), amount numeric(12,2), accrued_on date, accrued_at timestamp, cashier_shift_id uuid null references cashier_shifts(id), reversal_of uuid null references cashier_commission_entries(id))`; indexes on `(org_id, cashier_id, accrued_on)` and `(order_id)`
...
1. `buildOrderCommission` is pure and takes no database. Margin is `settledTotal` less the order's
   stock cost, less the order's own expenses, less the order's share of the day's expenses — the
   caller passes that share in, because it cannot be derived from the order alone. Pool is
   `max(0, margin) × completerRate / 100`.
...
5. **Accrue at shift close, not at order completion.** An order's share of the day's expenses
   depends on the day's total sales, so it is not knowable at the till. `computeCashierShiftBalanceSheet`
   already works the shift's allocation out at close; push that share down to the shift's individual
   orders in the same proportion used to derive it (each order's paid sales over the shift's paid
   sales for that day), then build each order's entries from the resulting margin. The allocation
   arithmetic is not re-invented — it is reused one level further down.
6. Store each order's `overhead_share` on its ledger row at shift close, even for a tick order that
   accrues nothing yet. K3 needs it months later when the customer pays, and by then the day it was
   sold on is long closed. Without this the figure would have to be recomputed from history, or
   silently dropped.
7. Accrue at shift close for every payment method **except** tick. Tick writes its share row but no
   commission; it accrues in K3 with `basis = 'credit_resolution'`, on the day the money arrives.
8. Personal use (K5) and any order with a zero or negative margin write no commission entries.
9. Rewrite `buildCashierShiftBalanceSheet` so `commissionAmount` is the sum of ledger entries whose
   `cashier_shift_id` is this shift, and `businessRetainedProfit` is `netSalesProfit` less that sum.
   Leave the rest of the sheet alone. The shift's own `netSalesProfit` must still equal what it
   computes today — the per-order shares are a decomposition of it, and must sum back to it.
10. Bump `CALCULATION_VERSION` to 2. Do not restate closed shifts: existing summaries keep version 1
    and their stored figures.

**Out of scope:**

- Changing what `netSalesProfit` means. It is confirmed correct as it stands (F3); this brief
  decomposes it per order, it does not redefine it.
...
- A tick order writes no commission entry at shift close, but does store its `overhead_share`.
- The per-order overhead shares for a shift sum back to that shift's `globalExpenseAllocation`.
- Two shifts on the same day split that day's expenses in proportion to their sales, and their
  orders split each shift's share in proportion to theirs.
...
3. A payment inserts a `credit_payments` row and reduces `amount_outstanding`. Status becomes
   ...
   Every payment records **how it was paid** — cash, card, transfer — and one debt may be cleared by
   several payments of different methods (£200 cash today, £150 card next week). The method is
   required, not optional: a cash settlement and a card settlement land differently in the drawer,
   and the Z-report cannot reconcile without knowing which.
4. Each payment accrues commission pro-rata: the share of the order's pool equal to the share of
   ...
   payment date. The pool is rebuilt from the `overhead_share` K2 stored at the original shift
   close — never recomputed from today's expenses, which belong to a different day's trading. A fully settled order must have accrued exactly its whole pool — reconcile the
...
- **Splitting a sale at the counter** — part cash, part card, part on tick, on one order. This
  brief covers instalments against a debt already given, not mixed tender at the point of sale.
  `orders.payment_method` holds exactly one tender, so that needs an `order_payments` table and a
  split-tender till screen. See *Open questions* item 0 — confirm which was meant before starting.
...
- Settled by £200 cash then £150 card: both payments carry their method, the drawer reconciles
  against the cash leg only, and the commission accrued across the two sums to the whole pool.
...
|-------|-----------|-----------|
| K1 | — | small |
| K2 | K1 | large — the ledger, per-order overhead shares, the shift-sheet rewrite |
...
Both former blockers are answered: overheads are in the base (daily figure only), and partial
payments release pro-rata with a tender method on each payment. The one thing still to confirm is
whether split tender at the counter was meant — see *Open questions* item 0. K3 is scoped to
instalments against a debt and can proceed either way; split tender would be an additional brief.
