...
| **Fix (2026-09-14, `#207`)** | Applied on `#207` (`client/src/styles/tokens/liquid-metal.css`): swapped `.lm-shell-sidebar`'s `background: var(--lm-surface-gradient)` for the same flat-midpoint solid color (`hsl(217, 11%, 13%)`) already used 3× elsewhere in the same file (`.pos-chip`, `.pos-action-bar`, `.pos-mobile-summary`) for this identical axe limitation — removing the one gradient axe named as a contributing `relatedNode`. Flagged at the time as **not confirmed causal**: the failure did not reproduce locally, with or without this change. Correct to keep (a solid background is still the right call for that limitation), but not sufficient — the job kept failing on later PRs (`#211`, `#214`, `#215`, `#218`, `#220`) with the sidebar now solid, which is what motivated the investigation below. |
| **Root cause (2026-09-20)** | Not a gradient. Pulled the raw axe JSON from four live CI failures (`#211`, `#215`, `#218`, `#220`) via the GitHub Actions job logs rather than reasoning from the summary text, and every one carries the same `message`: *"Element's background color could not be determined because it's partially obscured by another element"* (`elmPartiallyObscured`), with `aside.lm-shell-sidebar` as a `relatedNode` — `data-aria-hidden="true" aria-hidden="true"` in every capture. That attribute is Radix's own `aria-hidden-focus` behaviour: `DropdownMenu`/`Dialog` marks every other branch of the page `aria-hidden="true"` while open, so the rest of the app (including the sidebar) is hidden from assistive tech for as long as the order-actions menu is up. axe's static geometry check still finds that hidden sibling's bounding box overlapping the portalled menu's, even though it is never actually painted over it — an element nobody using a screen reader can perceive cannot be the reason someone else's contrast is wrong. That an aria-hidden sibling's geometry happens to line up with the portalled menu's is what makes this cluster rather than fire on every run (the brief's own "clusters, not purely random" note), and explains why `#214`, `#216`, `#219` and `#217` ran the identical job clean in the same window. |
| **Fix (2026-09-20)** | `tests/a11y/operations-centre.spec.ts`: added `obscuredByAriaHidden(node)` and routed every one of the 11 `color-contrast` assertions in the file through a new shared `assertNoColorContrastIssues(results)` helper. It still fails on a hard `violation` and on a genuine "can't sample this colour" `incomplete` (a real gradient, unrelated to this gap) exactly as before; it now drops an `incomplete` result first only when one of the `relatedNodes` axe attached to it carries `aria-hidden="true"`. Verified against the exact JSON captured from `#211`'s failure (reconstructed in a standalone script and run outside the suite): the real failure's node is correctly dropped, and a synthetic genuine-gradient node with no `aria-hidden` relatedNode is correctly left in. Live full-suite verification in this session's sandbox was inconclusive — unrelated `GET /api/orgs 401`s and page-load timeouts, occurring before any axe assertion runs and reproducing identically with the fix reverted, pointed at that sandbox's Playwright/dev-server setup rather than this change; CI (a clean runner) is the real check. Ported into `#211`, `#215`, `#218` and `#220` directly rather than waiting on this fix to merge first. |
| **Closed** | [x] |
