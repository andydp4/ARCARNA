...
export const requireActiveCashierShift: RequestHandler = async (req, res, next) => {
  try {
    ...
    // the time of sale. Trust closed shifts only when replay metadata proves the
    // queued sale happened while that shift was active.
    ...
    if (offlineCashierShiftId && offlineCashierId) {
      const [shift] = await db
        ...
        .limit(1);
      if (shift && shouldTrustSubmittedCashierShift(req.body, shift)) {
