...
function withOfflineReplayMetadata(data: any, timestamp: number): any {
  return {
    ...data,
  ...
  async syncOnline() {
    ...
    try {
      ...
      for (const order of unsyncedOrders) {
        try {
          await apiRequest('POST', '/api/orders', withOfflineReplayMetadata(order.data, order.timestamp));
        ...
        try {
          const data =
            mutation.type === 'ORDER_CREATE'
              ? withOfflineReplayMetadata(mutation.data, mutation.timestamp)
              : mutation.data;
