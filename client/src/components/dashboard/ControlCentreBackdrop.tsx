  ...
  return (
    ...
    >
      <style>{`
        .arc-cf {
          --arc-sphere: 260px;
          --arc-open: 16px;   /* where both doors come to rest — the same for each */
          --arc-overrun: calc(var(--arc-open) * 2.6);  /* how far the left one swings past first */
        ...
        /* Fixed shell at the poles. These never move, which is what stops the
           ...
           cut in a solid object, bounded by shell that stays put. It is the
           same full sphere as the doors, clipped to a band, so its etch
           registers exactly with theirs instead of reading as a separate
           strip laid across the top and bottom. */
        .arc-cf .arc-cap {
          position: absolute; inset: 0;
          background-image: var(--arc-etch), linear-gradient(103deg, #050b14 0%, #101c2e 34%, #16243a 50%, #0c1626 68%, #04080f 100%);
          background-size: 100% 100%, auto;
        ...
        /* Specular and environment reflection. Above the doors and fixed,
           ...
           The specular is one soft oval, not a pip inside a glow. An earlier
           pair sat on different centres with a tight, near-opaque core, and
           the bright spot read as a speck of dirt on the sphere rather than
           a reflection in it. Wider than tall, so it lies along the surface
           instead of hovering in front of it, and it fades out well short of
           its own edge — a hard stop is what gives a highlight away.

           There is no horizon band. A reflected horizon is the conventional
           way to say "polished", but on a sphere this size it draws a level
           line clean across the middle and the object goes flat — the same
           complaint the etch pattern drew. Roundness comes from the specular,
           the bounce below right, and the limb falling away in .arc-form. */
        .arc-cf .arc-sheen {
          position: absolute; inset: 0; border-radius: 50%;
          mix-blend-mode: screen;
          background:
            radial-gradient(ellipse 24% 15% at 32% 25%, rgba(234,245,255,0.30) 0%, rgba(200,226,255,0.16) 34%, rgba(146,188,238,0.06) 62%, transparent 88%),
            radial-gradient(ellipse 58% 46% at 33% 27%, rgba(178,208,245,0.12) 0%, rgba(110,160,220,0.04) 50%, transparent 82%),
        ...
        /* The left door swings well past its stop and draws back level with
           the right, so the mechanism reads as driven rather than eased into
           place. It settles on --arc-open like its opposite number: the
           overrun is the flourish, not the resting position. */
        @keyframes arc-cf-door-l {
          0%,18% { transform: translateX(0); }
          52% { transform: translateX(calc(var(--arc-overrun) * -1)); }
