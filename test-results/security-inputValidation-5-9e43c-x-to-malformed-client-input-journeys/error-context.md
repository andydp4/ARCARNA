...
- Name: tests/journeys/security/inputValidation.spec.ts >> 5.7 input validation >> no route may answer 5xx to malformed client input
- Location: tests/journeys/security/inputValidation.spec.ts:561:8

# Error details

```
Error: malformed input must never produce a server error
...
- Array []
+ Array [
+   "POST /api/suppliers [name=100k chars] (server/routes/suppliers.ts:66) → 500",
+   "POST /api/suppliers [name=unicode + emoji + NUL] (server/routes/suppliers.ts:66) → 500",
+   "POST /api/suppliers [leadTimeDays=MAX_SAFE_INTEGER] (server/routes/suppliers.ts:66) → 500",
+   "POST /api/suppliers [leadTimeDays=beyond MAX_SAFE_INTEGER] (server/routes/suppliers.ts:66) → 500",
+   "POST /api/suppliers [leadTimeDays=1e308] (server/routes/suppliers.ts:66) → 500",
+   "POST /api/suppliers [minOrderValue=MAX_SAFE_INTEGER] (server/routes/suppliers.ts:66) → 500",
+   "POST /api/suppliers [minOrderValue=beyond MAX_SAFE_INTEGER] (server/routes/suppliers.ts:66) → 500",
+   "POST /api/suppliers [minOrderValue=1e308] (server/routes/suppliers.ts:66) → 500",
+   "POST /api/product-suppliers [packSize=MAX_SAFE_INTEGER] (server/routes/suppliers.ts:112) → 500",
+   "POST /api/product-suppliers [packSize=beyond MAX_SAFE_INTEGER] (server/routes/suppliers.ts:112) → 500",
+   "POST /api/product-suppliers [packSize=1e308] (server/routes/suppliers.ts:112) → 500",
+   "POST /api/product-suppliers [costPrice=zero] (server/routes/suppliers.ts:112) → 500",
+   "POST /api/product-suppliers [costPrice=fractional] (server/routes/suppliers.ts:112) → 500",
+   "POST /api/product-suppliers [costPrice=MAX_SAFE_INTEGER] (server/routes/suppliers.ts:112) → 500",
+   "POST /api/product-suppliers [costPrice=beyond MAX_SAFE_INTEGER] (server/routes/suppliers.ts:112) → 500",
+   "POST /api/product-suppliers [costPrice=1e308] (server/routes/suppliers.ts:112) → 500",
+   "POST /api/inventory/transfers [huge quantity] (server/routes/inventoryTransfers.ts:70) → 500",
+   "PATCH /api/suppliers/:id [leadTimeDays as string] (server/routes/suppliers.ts:81 — req.body passed straight to the service) → 500",
+   "PATCH /api/suppliers/:id [name as number] (server/routes/suppliers.ts:81 — req.body passed straight to the service) → 500",
+   "PATCH /api/suppliers/:id [name 100k chars] (server/routes/suppliers.ts:81 — req.body passed straight to the service) → 500",
+   "PATCH /api/suppliers/:id [unknown column injection] (server/routes/suppliers.ts:81 — req.body passed straight to the service) → 500",
+   "POST /api/products [empty body] (server/routes/products.ts:72 — no schema at the route) → 500",
+   "POST /api/customers [empty body] (server/routes/customers.ts:77) → 500",
+   "POST /api/customers [email 100k chars] (server/routes/customers.ts:77) → 500",
+   "POST /api/gift-cards [amount MAX_SAFE_INTEGER] (server/routes/giftCards.ts:64 — issueSchema.parse) → 500",
+ ]
```
