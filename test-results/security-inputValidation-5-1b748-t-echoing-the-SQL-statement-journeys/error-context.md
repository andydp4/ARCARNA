...
- Name: tests/journeys/security/inputValidation.spec.ts >> 5.7 input validation >> an over-length field must be rejected without echoing the SQL statement
- Location: tests/journeys/security/inputValidation.spec.ts:622:8

# Error details

```
Error: an over-length string is a client error, not a server error
