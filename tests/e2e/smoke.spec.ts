...
test.describe("API smoke", () => {
  test("GET /midnight/api/health returns ok", async ({ request }) => {
    const res = await request.get("/midnight/api/health");
    expect(res.ok()).toBeTruthy();
    const body = (await res.json()) as { ok?: boolean };
    expect(body.ok).toBe(true);
  });

  test("GET /midnight/api/auth/runtime exposes dev bypass in e2e", async ({ request }) => {
    const res = await request.get("/midnight/api/auth/runtime");
    expect(res.ok()).toBeTruthy();
